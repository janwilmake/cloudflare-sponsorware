...
// Type definition for rate limit headers
...
export class RatelimitDO extends DurableObject {
   ...
   */
  async checkRequest(options?: RateLimitOptions): Promise<{
    waitTime: number;
  ...
  private async setResetAlarm(): Promise<void> {
...
> {
  ...
  const ratelimiterStub: any = env.RATELIMIT_DO.get(
    env.RATELIMIT_DO.idFromName(clientIp),
  );

  // Call the checkRequest method directly on the stub using RPC
  const { waitTime, ratelimitHeaders } = await ratelimiterStub.checkRequest(
    options,
  );
